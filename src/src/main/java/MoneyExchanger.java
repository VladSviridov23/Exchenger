...
import src.main.java.ExchangeRate;
...
//using singleton pattern
	...
	public BigDecimal buy(BigDecimal amount, String targetCurrency) {
		//buy for UAH with selected currency
		...
		try {
			...
			if (amount != null || targetCurrency != null) {
				...
				System.out.println("It will cost you " + tempFinalPrice + " UAH");
			}
		...
		try {
			...
			if (amountToSell != null || currencyToExchange != null) {
				...
				System.out.println("You want to sell: " + amountToSell + " " + currencyToExchange + "\n" +
						...
						"You will get " + tempFinalPrice + " UAH");
	...
	public Map<String, ExchangeRate> initializeHashMap() {
		hashmap.put("USD", new ExchangeRate(new BigDecimal("27.6"), new BigDecimal("27.7")));
		hashmap.put("EUR", new ExchangeRate(new BigDecimal("29.1"), new BigDecimal("29.7")));
		hashmap.put("GBP", new ExchangeRate(new BigDecimal("33.4"), new BigDecimal("33.3")));
		hashmap.put("CHF", new ExchangeRate(new BigDecimal("27.8"), new BigDecimal("28.25")));
	...
	public void startExchange() {
		...
		try {
			if (enterExchanger.equals("buy")) {
				System.out.println("Which currency do you want to buy?(USD,EUR,GPB,CHF)");
				...
				System.out.println("Which currency do you want to sell?(USD,EUR,GPB,CHF)");
				String chooseCurrency = scanner.next();
